"""Unified multi-instance VPN panel layer."""
...
async def get_catalog(p):
    ...
    if p.get('panel_type') == 'rebecca':
        ...
        for i,x in enumerate(items):
            ...
            ref=str(x['id']); name=x.get('name') or f'Service {ref}'
            hosts=int(x.get('host_count') or 0)
            users=int(x.get('user_count') or 0)
            label=f'🦋 {name} | {hosts} Host | {users} User'
            out.append({'idx':i,'ref':ref,'name':name,'label':label[:60]})
        return (out,'موفق') if out else ([],f'هیچ Service قابل استفاده‌ای در {panel_label(p)} پیدا نشد.')
    ...
    for i,x in enumerate(items):
        ...
        label=f'📦 {name} (id: {ref})'; out.append({'idx':i,'ref':ref,'name':name,'label':label[:60]})
...
async def create_service(p,username,remote_ref,volume_gb=None,days=None,device_limit=None):
    ...
    if p.get('panel_type') == 'rebecca':
        try: sid=int(remote_ref)
        except Exception:return False,None,None,None,'Service ID Rebecca معتبر نیست.'
        ok,data,msg=await c.create_user_custom(p,sid,username,volume_gb,days,device_limit=device_limit)
    else:
        tid=int(remote_ref)
