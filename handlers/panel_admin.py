...
async def map_vip_plan(callback:types.CallbackQuery,state:FSMContext):
    ...
    if p: await _catalog(callback,state,p,'vip_plan',int(planid),f'یک {"Service" if p.get("panel_type")=="rebecca" else "Template"} از {_label(p)} را برای این پلن انتخاب کن:')
...
async def map_category(callback:types.CallbackQuery,state:FSMContext):
    ...
    if p: await _catalog(callback,state,p,'vip_category',int(cid),f'یک {"Service" if p.get("panel_type")=="rebecca" else "Template"} از {_label(p)} را به‌عنوان مقصد پیش‌فرض کل این دسته انتخاب کن:')

@router.callback_query(F.data.startswith('vpnmapcustom|'))
async def map_custom(callback:types.CallbackQuery,state:FSMContext):
    if not _is_admin(callback.from_user.id):return
    pid=int(callback.data.split('|')[1]); p=db.get_vpn_panel(pid)
    if p: await _catalog(callback,state,p,'custom_build',0,f'یک {"Service" if p.get("panel_type")=="rebecca" else "Template"} از {_label(p)} را برای «بساز سرویس خودت» انتخاب کن:')

@router.callback_query(F.data.startswith('vpnmapfreetest|'))
async def map_test(callback:types.CallbackQuery,state:FSMContext):
    if not _is_admin(callback.from_user.id):return
    pid=int(callback.data.split('|')[1]); p=db.get_vpn_panel(pid)
    if p: await _catalog(callback,state,p,'free_test',0,f'یک {"Service" if p.get("panel_type")=="rebecca" else "Template"} از {_label(p)} را برای تست رایگان انتخاب کن:')
